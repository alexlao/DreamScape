...
public abstract class ScrollWorld extends World
{
    ...
    private final ArrayList<ScrollingActor> objects;
    private final ArrayList<ScrollingActor> camFollowers;
    ...
    public ScrollWorld(int width, int height, int cellSize, int fullWidth, int fullHeight)
    {
        ...
        objects = new ArrayList<ScrollingActor>();
        camFollowers = new ArrayList<ScrollingActor>();
     ...
     */
    public void addCameraFollower(ScrollingActor cameraFollower, int x, int y)
    {
    ...
    {
        if (object instanceof ScrollingActor) {
            if (x >= fullWidth)
                x = fullWidth -1;
            else if (x < 0)
                x = 0;
            if (y >= fullHeight)
                ...
                y = 0;
            ScrollingActor sa = (ScrollingActor) object;
    ...
    public void removeObject(Actor object)
    {
        ...
        if (object instanceof ScrollingActor) {
            ScrollingActor a = (ScrollingActor) object;
            objects.remove(a);
    ...
    public void setCameraLocation(int x, int y)
    {
        ...
        camX = x;
        camY = y;
        for (ScrollingActor a : objects)
            a.setLocation(a.getX() -dx, a.getY() -dy);
        for (ScrollingActor a : camFollowers)
